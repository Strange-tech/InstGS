...
def visualize_two_pointclouds(pc1, pc2):
    # 转为 open3d 点云对象
...
if __name__ == "__main__":
    ...
    for temp_map in scene_graph:
        ...
        for inst in temp_map["instances"]:
            ...
            instances_pc.append(gs.get_xyz.detach().cpu().numpy())
        ...
        template_gs.merge(all_instances, mode="all")

        # 3. 为每个实例初始化 SH offset（同 shape）
        xyz_offsets = []
        for idx, inst in enumerate(temp_map["instances"]):
            inst_xyz = instances_pc[idx]
            tree = cKDTree(inst_xyz)
            t = torch.from_numpy(np.array(inst["transform"]).T).float().to("cuda")
            t_inv = torch.inverse(t)
            fused_xyz = geom_transform_points(template_gs.get_xyz, t_inv)
            fused_xyz = fused_xyz.detach().cpu().numpy()
            _, idxs = tree.query(fused_xyz, k=1)
            xyz_offset = inst_xyz[idxs] - fused_xyz
            xyz_offsets.append(torch.from_numpy(xyz_offset))
            # visualize_two_pointclouds(fused_xyz, inst_xyz)
        xyz_offsets = torch.stack(xyz_offsets, dim=0).to("cuda")
        xyz_offsets.requires_grad = True
        # for inst in all_instances:
        #     inst_xyz = inst.get_xyz.detach().cpu().numpy()
        #     tree = cKDTree(inst_xyz)
        #     _, idxs = tree.query(fused_xyz, k=1)
        #     xyz_offset = inst_xyz[idxs] - fused_xyz
        #     xyz_offsets.append(torch.from_numpy(xyz_offset))
        ...
        template_gs.save_ply(
            f"./output/{SCENE_NAME}/inst_gs_{template_id}_init.ply",
            instancing=False,)
